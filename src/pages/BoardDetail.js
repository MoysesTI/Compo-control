...
export default function BoardDetail() {
    ...
    const fetchBoardData = async () => {
      try {
        ...
        const columnsPromises = columnsSnapshot.docs.map(async (columnDoc) => {
          ...
          const cardsRef = collection(db, "boards", id, "columns", columnDoc.id, "cards");
          let cardsQuery;
          ...
          const cards = cardsSnapshot.docs
            ...
            .filter(card => 
              ...
              (card.visibility === 'private' && card.assignedTo?.includes(currentUser.uid))
            );
  ...
  const handleAddCard = async (columnId) => {
    ...
    try {
      ...
      const cardsRef = collection(db, "boards", id, "columns", columnId, "cards");
  ...
  const handleEditCardSave = async () => {
    ...
    try {
      ...
      const cardRef = doc(db, "boards", id, "columns", columnId, "cards", cardId);
    ...
    try {
      ...
      if (source.droppableId === destination.droppableId) {
        ...
        newCards.forEach((card, index) => {
          const cardRef = doc(db, "boards", id, "columns", source.droppableId, "cards", card.id);
        ...
        const sourceCardRef = doc(db, "boards", id, "columns", source.droppableId, "cards", draggableId);
        ...
        if (sourceCardSnapshot.exists()) {
          ...
          const destCardsRef = collection(db, "boards", id, "columns", destination.droppableId, "cards");
  ...
  if (error) {
    return (
      <Box sx={{ p: 3 }}>
        ...
        <Button variant="contained" sx={{ mt: 2 }} onClick={() => navigate('/boards')}>
          Voltar para Quadros
        </Button>
      </Box>
    );
  ...
  return (
    <Box sx={{ padding: 3 }}>
      ...
      >
        <MenuItem onClick={() => handleEditCardOpen(selectedCard, selectedCard?.columnId)}>
          <EditIcon fontSize="small" sx={{ mr: 1 }} />
          Editar Cartão
        </MenuItem>
        <MenuItem onClick={() => handleAssignMembersOpen(selectedCard, selectedCard?.columnId)}>
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            onChange={(e) => setEditCardData({ ...editCardData, dueDate: e.target.value })}
