...
export default function EnhancedBoardDetail() {
  ...
  const handleDragEnd = async (result) => {
    ...
    try {
      setProcessingAction(true);
      console.log(`Moving card ${draggableId} from column ${source.droppableId} to ${destination.droppableId}`);
  ...
  const getLabelColor = (label) => {
    ...
    if (boardData?.labels) {
      const customLabel = boardData.labels.find(l => l.name === label || l.id === label);
      if (customLabel) {
  ...
  const handleSaveColumnEdit = async (updatedColumnData) => {
    ...
    try {
      ...
      await columnService.updateColumn(boardId, selectedColumn.id, updatedColumnData);
  ...
  const ManageLabelsDialog = ({ open, onClose, labels = [], onCreateLabel, onEditLabel, onDeleteLabel, loading }) => {
          ...
          <List>
            {labels.map((label) => (
              <ListItem
                ...
                secondaryAction={
                  <Box>
                    <IconButton edge="end" onClick={() => handleStartEdit(label)}>
                      <EditIcon />
                    </IconButton>
  ...
  const ShareBoardDialog = ({ open, onClose, boardData, teamMembers, loading }) => {
    const [selectedMembers, setSelectedMembers] = useState(boardData?.members || []);
      ...
      <Box sx={{ padding: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, overflowX: 'auto', pb: 2 }}>
          {[1, 2, 3, 4].map((i) => (
            ...
            >
              <Box sx={{ p: 2, bgcolor: 'secondary.light', borderTopLeftRadius: 8, borderTopRightRadius: 8 }}>
    ...
    return (
      <Box sx={{ p: 3 }}>
        <Alert 
          ...
          sx={{ mb: 2 }}
          action={
            <Button color="inherit" size="small" onClick={() => navigate('/boards')}>
              Voltar
            </Button>
          }
        >
  ...
  return (
    <Box sx={{ padding: 3 }}>
                      ...
                      <Draggable key={card.id} draggableId={card.id} index={index}>
                        {(provided, snapshot) => (
                          ...
                          >
                            ...
                            <Tooltip 
                              title={card.visibility === 'private' ? 'Visível apenas para membros atribuídos' : 'Visível para todos'}
                            >
                            ...
                            <IconButton 
                              ...
                              onClick={(e) => handleCardMenuOpen(e, card, column.id)}
                            >
                              <MoreVertIcon fontSize="small" />
                            </IconButton>
                            ...
                            <Box sx={{ mb: 1.5, display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                              {card.labels && card.labels.map((label, idx) => (
                                <Chip
                                  ...
                                  label={
                                    boardData?.labels
                                      ? boardData.labels.find(l => l.id === label)?.name || label
                                      : label
                                  }
                                  size="small"
                                  sx={{
      ...
      >
        <MenuItem onClick={() => handleEditCardClick(selectedCard, selectedCard?.columnId)}>
          <EditIcon fontSize="small" sx={{ mr: 1 }} />
          Editar Cartão
        </MenuItem>
        <MenuItem onClick={() => handleDuplicateCard(selectedCard, selectedCard?.columnId)}>
        ...
        <MenuItem onClick={() => handleArchiveCard(selectedCard, selectedCard?.columnId)}>
          <ArchiveIcon fontSize="small" sx={{ mr: 1 }} />
          Arquivar Cartão
        </MenuItem>
        <MenuItem onClick={handleDeleteCardClick} sx={{ color: 'error.main' }}>
          <DeleteIcon fontSize="small" sx={{ mr: 1 }} />
          Excluir Cartão
        </MenuItem>
      </Menu>
